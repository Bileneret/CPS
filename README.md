...
Проект "Прогнозування погоди та аналіз якості прогнозів" — це програмна система, яка використовує машинне навчання для прогнозування погодних умов (температура, опади, вітер, тиск) на основі історичних даних, збережених у базі даних. 

2. **Навіщо це?**

Мета проекту — створити інструмент для автоматичного прогнозування погоди та оцінки його якості. Основні цілі:
- **Прогнозування**: Допомагає передбачати погоду (у нашому прикладі температура), що корисно для планування (аграріям чи метеорологам). Та навіть базово запланувати прогулянку по місту :)
- **Практичне застосування**: Прогноз погоди є прогнозом погоди. Цим ми користуємось кожен день, щоб просто вийти на вулицю не в шортах в сніг. <small>(для шорт треба ще й мізки використовувати, но то важко)</small>

3. **Це КФС?**

- **Фізичний компонент**: Дані про погоду (температура, опади, вітер) відображають реальні фізичні явища в атмосфері, зібрані з датчиків чи метеостанцій (ці дані представлені у файлі CSV у вигляді набору числових даних).

- **Кібернетичний компонент**:  

  - `main.py` використовує `Random Forest` для обробки даних і генерації прогнозу температури.
  - Візуалізація в `Matplotlib` відображає результати.

- **Взаємодія**: Програма бере реальні дані, зібрані в Базелі (фізичний вимір), обробляє їх (кібернетична частина) і видає прогноз, який може впливати вже на інші реальні дії (наприклад, планування сільського господарства, посіви, будівництво і будь-що).

</details>

### Деталі проекту "Прогнозування погоди"

Цей проект дозволяє прогнозувати погодні умови, використовуючи історичні дані та машинне навчання. Ось як він працює і що робить:

1. **Завантаження даних:**
   - Дані беруться з таблиці `weather_data` у MySQL, яку заповнюють через `csvindb.py` з файлу `dataexport_20250508T130923.csv`.
   - Приклад колонок: `timestamp`, `Basel Temperature [2 m elevation corrected]`, `Basel Precipitation Total` тощо.

2. **Підготовка даних:**
   - Додаються часові ознаки (`hour`, `day_of_year`) та їх циклічні представлення (`sin_hour`, `cos_hour`).
   - Створюються лаги (попередні значення температури, опадів тощо за останні 24 години), щоб модель могла "згадати" минулу погоду.

3. **Навчання моделі:**
   - Використовується Random Forest (ансамбль дерев рішень), який вчиться на даних до 06.03.2024.
   - Модель прогнозує чотири змінні: `temperature`, `precipitation`, `wind_gust`, `pressure`.
   - Точність оцінюється через MSE, наприклад, для температури виводиться значення помилки.

4. **Прогноз і візуалізація:**
   - Прогнозується погода на 07.03.2024 на основі останніх даних від 06.03.2024.
   - Графік порівнює реальні значення температури за 06.03.2024 з прогнозованими.
   - Результати зберігаються у файлі `results.txt` і базі `predictions`.

5. **Приклад використання:**
   - Аграрій може використати прогноз температури та опадів, щоб вирішити, коли поливати поля.
   - Метеоролог може проаналізувати MSE, щоб покращити модель.

### Потенційні покращення
- Додати більше змінних (наприклад, вологість, хмарність) у прогноз.
- Використати глибші моделі (наприклад, LSTM) для кращої роботи з часовими рядами.
- Додати автоматичне оновлення даних із зовнішніх API (наприклад, OpenWeatherMap).

Цей проект — приклад, як можна комбінувати дані, машинне навчання та зберігання для практичних задач прогнозування!
